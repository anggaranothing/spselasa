 ...
 */
public class FrameGroup extends javax.swing.JInternalFrame {

    private List<Group>         modelRecord     = new ArrayList<Group>();
    private IfGroupDao          objectDao       = null;
    ...
    private void initDao() {  
        String url = String.format( "rmi://%s:%d/%s" , ClientConfig.getRmiHost() , ClientConfig.getRmiPort() , ClientConfig.RMI_GROUP_NAME );
        jBusyLabel.setVisible( true );
        try {
            
            // ambil referensi dari remote object yg ada di server
            objectDao = ( IfGroupDao ) Naming.lookup(url);
            
        } catch (NotBoundException ex) {
            Helper.logThrowable( "FrameGroup initDao() failed!", ex );
    ...
    private void loadRecord() {
        ...
        try {
            ...
            Helper.logThrowable( "FrameGroup loadRecord() failed!", ex );
        }
    ...
    private void fillToTable( boolean isEditData, Group group , DefaultTableModel tableModel ) {
        if ( !isEditData ) { // data baru
            Object[] objects = new Object[ tableModel.getColumnCount() ];

            objects[0] = group.getId();
            ...
            if( row > -1 ) {
                ...
                tableModel.setValueAt( group.getId(),   row, 0);
                tableModel.setValueAt( group.getName(), row, 1);
            }
        }
    }
    ...
    private void doFindKeyword( String keyword , boolean isPrevious ) {
        ...
        new Thread(new Runnable(){ public void run(){
            ...
            while( whileCondition ) {
                ...
                Group group = (Group) modelRecord.get( lastFindIndex );
    ...
    private void doAddEditRecord() {
        ...
        groupObj.setId(    Integer.valueOf( tfDlgId.getText() ) );
        groupObj.setName(  Helper.nullifyEmpty( tfDlgNama.getText() ) );
        
        try {
            if( !isEditRecord )
                objectDao.create(groupObj);
            else
                objectDao.update(groupObj);
    ...
    private void doDeleteRecord() {
        ...
        Group groupObj = new Group();
        groupObj.setId( modelRecord.get( getRecordIndexOnTable() ).getId() );
        
        try {
            objectDao.delete( groupObj );
            ...
            .addGroup(dialogAddEditLayout.createSequentialGroup()
                ...
                .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(dialogAddEditLayout.createSequentialGroup()
                        ...
                        .addGap(107, 107, 107)
                        .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(tfDlgId, javax.swing.GroupLayout.DEFAULT_SIZE, 188, Short.MAX_VALUE)
