 ...
 */
public class FrameCfLevel extends javax.swing.JInternalFrame {

    private List<CfLevel>       modelRecord     = new ArrayList<CfLevel>();
    private IfCfLevelDao        objectDao       = null;
    ...
    private void initDao() {  
        String url = String.format( "rmi://%s:%d/%s" , ClientConfig.getRmiHost() , ClientConfig.getRmiPort() , ClientConfig.RMI_CFLEVEL_NAME );
        jBusyLabel.setVisible( true );
        try {
            
            // ambil referensi dari remote object yg ada di server
            objectDao = ( IfCfLevelDao ) Naming.lookup(url);
            
        } catch (NotBoundException ex) {
            Helper.logThrowable( "FrameCFLevel initDao() failed!", ex );
    ...
    private void loadRecord() {
        ...
        try {
            ...
            Helper.logThrowable( "FrameCFLevel loadRecord() failed!", ex );
        }
    ...
    private void fillToTable( boolean isEditData, CfLevel cfl , DefaultTableModel tableModel ) {
        if ( !isEditData ) { // data baru
            Object[] objects = new Object[ tableModel.getColumnCount() ];

            objects[0] = cfl.getValue();
            objects[1] = cfl.getDescription();
            ...
            if( row > -1 ) {
                ...
                tableModel.setValueAt( cfl.getValue(),       row, 0);
                tableModel.setValueAt( cfl.getDescription(), row, 1);
            }
        }
    }
    ...
    private void doFindKeyword( String keyword , boolean isPrevious ) {
        ...
        new Thread(new Runnable(){ public void run(){
            ...
            while( whileCondition ) {
                ...
                CfLevel cfl = (CfLevel) modelRecord.get( lastFindIndex );
                
                if( Helper.isFloat( keyword.toLowerCase().trim() ) == false) {
                    if( cfl.getDescription().toLowerCase().contains( keyword.toLowerCase() ) ) {
                        isFound = true;
                        isOkayToChangePage = true;
                    }
                }
                else {
                    if( cfl.getValue() == Float.valueOf( keyword.trim() ) ) {
                        isFound = true;
                        isOkayToChangePage = true;
                    }
                }
    ...
    private void doAddEditRecord() {
        ...
        CfLevel cflObj = new CfLevel();
        cflObj.setValue(       (float) spDlgValue.getValue() );
        cflObj.setDescription( Helper.nullifyEmpty( tfDlgDesc.getText() ) );
        
        try {
            if( !isEditRecord )
                objectDao.create(cflObj);
            else
                objectDao.update(cflObj);
    ...
    private void doDeleteRecord() {
        ...
        CfLevel cflObj = new CfLevel();
        cflObj.setValue( modelRecord.get( getRecordIndexOnTable() ).getValue() );
        
        try {
            objectDao.delete( cflObj );
    ...
    private void initComponents() {
        ...
        dialogAddEdit.setMinimumSize(new java.awt.Dimension(360, 195));
        dialogAddEdit.setModal(true);
        dialogAddEdit.setResizable(false);
        dialogAddEdit.setSize(new java.awt.Dimension(360, 195));
        ...
        tfDlgDesc.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        tfDlgDesc.setText("jTextField2");
        ...
        spDlgValue.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        spDlgValue.setModel(new javax.swing.SpinnerNumberModel(Float.valueOf(0.0f), Float.valueOf(-1.0f), Float.valueOf(1.0f), Float.valueOf(0.1f)));
            ...
            .addGroup(dialogAddEditLayout.createSequentialGroup()
                ...
                .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(dialogAddEditLayout.createSequentialGroup()
                        ...
                        .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(tfDlgDesc, javax.swing.GroupLayout.DEFAULT_SIZE, 200, Short.MAX_VALUE)
                            .addComponent(spDlgValue)))
                    .addGroup(dialogAddEditLayout.createSequentialGroup()
        ...
        dialogAddEditLayout.setVerticalGroup(
            ...
            .addGroup(dialogAddEditLayout.createSequentialGroup()
                ...
                .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabel7)
                    .addComponent(spDlgValue, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addGap(18, 18, 18)
                .addGroup(dialogAddEditLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(tfDlgDesc, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jLabel11))
                .addGap(18, 18, 18)
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Certainty Factor Level Data");
    ...
    private void dialogAddEditComponentShown(java.awt.event.ComponentEvent evt) {//GEN-FIRST:event_dialogAddEditComponentShown
        ...
        String  dlgTitle       = "Add New Record",
                dlgDesc        = "";
        float   dlgValue       = 0;
        boolean dlgValEditable = true;
        
        if( isEditRecord && getRecordIndexOnTable() >= 0 ) {
            dlgTitle = "Edit Selected Record";
            dlgValue = modelRecord.get( getRecordIndexOnTable() ).getValue();
            dlgDesc  = modelRecord.get( getRecordIndexOnTable() ).getDescription();
            dlgValEditable = false;
        }
        
        dialogAddEdit.setTitle( dlgTitle );
        tfDlgDesc.setText(   dlgDesc );
        spDlgValue.setValue( dlgValue );
        ((DefaultEditor) spDlgValue.getEditor()).getTextField().setEditable(dlgValEditable);
        spDlgValue.setEnabled(dlgValEditable);
    ...
    private void btnDlgConfirmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDlgConfirmActionPerformed
        // name is empty?
        if( Helper.nullifyEmpty( tfDlgDesc.getText() ) == null ) {
            JOptionPane.showMessageDialog( this,
    ...
    private javax.swing.JSpinner spDlgValue;
    private javax.swing.JTable tblRecord;
    private javax.swing.JTextField tfDlgDesc;
